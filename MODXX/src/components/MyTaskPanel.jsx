    ...
    <div className="bg-gray-800 rounded-lg shadow-inner p-4 overflow-y-auto max-h-[40vh] w-full mb-8">
      ...
      {loading ? (
        ...
        <ul className="space-y-2">
          {tasks.map((task) => (
            <li key={task.id} className="bg-gray-700 rounded p-2 flex flex-col gap-1">
              <span className="font-semibold text-white">{task.title}</span>
              <span className="text-xs text-gray-300">{task.description}</span>
              <span className="text-xs text-gray-400">Deadline: {task.deadline ? new Date(task.deadline).toLocaleDateString() : "N/A"}</span>
