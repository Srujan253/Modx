...
const ProjectTasks = ({ projectId, members }) => {
  const { user } = useAuth();
  ...
  const handleMarkDone = async (taskId) => {
    ...
    setError("");
    try {
      await axiosInstance.patch(`/project/${projectId}/tasks/${taskId}`, { status: "done" });
      fetchTasks();
    } catch (err) {
  ...
  const visibleTasks = isMember ? tasks.filter((t) => t.assigned_to === user.id) : tasks;
          ...
          {visibleTasks.map((task) => (
            <li key={task.id} className="py-2">
              ...
              <div className="text-sm text-gray-600">
                Assigned to: {members.find((m) => m.id === task.assigned_to)?.full_name || task.assigned_to}
              </div>
