...
exports.createTask = async (req, res) => {
  ...
  try {
    ...
    if (userRole !== 'leader' && userRole !== 'mentor') {
      return res.status(403).json({ error: 'Only leader or mentor can assign tasks' });
    }

    // Check if assigned_to is a member of the project
    const member = await db.query('SELECT * FROM project_members WHERE project_id = $1 AND member_id = $2 AND status = $3', [projectId, assigned_to, 'accepted']);
    if (!member.rows.length) return res.status(400).json({ error: 'Assigned user is not a project member' });
    ...
    const result = await db.query(
      ...
      [projectId, assigned_to, assigned_by, title, description, status || 'pending', deadline]
    );
...
exports.deleteTask = async (req, res) => {
  const { projectId, taskId } = req.params;
  try {
    const result = await db.query('DELETE FROM tasks WHERE id = $1 AND project_id = $2 RETURNING *', [taskId, projectId]);
    if (!result.rows.length) return res.status(404).json({ error: 'Task not found' });
